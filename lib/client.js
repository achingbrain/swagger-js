...
SwaggerClient.prototype.buildFromSpec = function (response) {
  ...
  _.forEach(response.paths, function (pathObj, path) {
    ...
    _.forEach(supportedOperationMethods, function (method) {
      if (pathObj[method] && pathObj[method].length > 1 && !self.options.allowMultiples) {
        helpers.log('Multiple operations were found for \'' + method + '\' \'' + path + '\'')
        helpers.log('This is not compliant with Swagger 2.0, please set `allowMultiples` to true to enable this.');
        helpers.log('See https://github.com/swagger-api/swagger-spec/issues/182 for more information')

        pathObj[method] = pathObj[method].slice(0, 1)
      }

      _.forEach(pathObj[method], function (operation) {
